import XCTest
@testable import Framebook

@MainActor
final class FramebookTests: XCTestCase {
    var store: Store!
    ...
    func testAddIncreasesCount() {
        ...
        store.add(Job(title: "Test Entry"))
        XCTAssertEqual(store.items.count, before + 1)
    }

    func testDeleteDecreasesCount() {
        store.add(Job(title: "To Delete"))
        let before = store.items.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        store.isPro = false
        store.items = (0..<Store.freeLimit).map { _ in Job(title: "x") }
